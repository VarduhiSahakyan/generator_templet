...
SET @BankB_1 = 'Bankard_Credit';
SET @BankB_2 = 'RCBC_Debit';
SET @IssuerCode = '00006';
SET @SubIssuerCode_1 = '00006';
SET @SubIssuerCode_2 = '00018';
...
set @id_subissuer_2 = (SELECT ID from SubIssuer where FK_ID_ISSUER = @id_issuer and code = @SubIssuerCode_2);
...
delete from CustomItem where FK_ID_CUSTOMITEMSET IS NULL OR id in (select id from (select ci.id from CustomItem ci,
																				...
																				and pr.ID_RULE = r.ID
																				and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2)
																				and (ci.fk_id_network is NULL or ci.fk_id_network = @NetworkVID or ci.fk_id_network = @NetworkMID or ci.fk_id_network = @NetworkJCB)) as temp);

delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2) and fk_id_network in (@NetworkVID, @NetworkMID, @NetworkJCB));

delete from BinRange where FK_ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2) and fk_id_network in (@NetworkVID, @NetworkMID,@NetworkJCB);

delete from Network_SubIssuer where ID_SUBISSUER in (@id_subissuer_1, @id_subissuer_2) and id_network in (@NetworkVID, @NetworkMID,@NetworkJCB);
...
where ID in (select id
			...
			and (CP.DESCRIPTION like CONCAT('%(', @BankB_1, ')%')
			or CP.DESCRIPTION like CONCAT('%(', @BankB_2, ')%'))) as temp);

-- delete the profile set / custompage layout mapping
delete from CustomPageLayout_ProfileSet where PROFILESET_ID in (@id_profile_set_1, @id_profile_set_2);

delete from CustomPageLayout where DESCRIPTION like CONCAT('%(', @BankB_1, ')%')
								or DESCRIPTION like CONCAT('%(', @BankB_2, ')%') ;
...
delete from CustomItemSet where fk_id_subIssuer in (@id_subissuer_1, @id_subissuer_2);
...
where pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2);

SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2);
...
delete from RuleCondition where ID in (SELECT id from (SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
													   where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in
																											(@id_profile_set_1, @id_profile_set_2)) as temp);
...
delete from ProfileSet_Rule where ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2);

delete from Rule where find_in_set(id, @id_rule);

delete from ProfileSet where id in (@id_profile_set_1, @id_profile_set_2);
...
delete from SubIssuer where ID in (@id_subIssuer_1, @id_subIssuer_2);

delete from Issuer where ID = @id_issuer;

delete from Image where name like  '%RCBC%'
								or name like '%Bankard%';

delete from CryptoConfig where description = 'CryptoConfig for RCBC Debit';
