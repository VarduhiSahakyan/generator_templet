...
delete from CustomItem where fk_id_customItemSet = @customItemSetPassword and pageTypes = 'HELP_PAGE' and ordinal in (1,2,3,11);

set @customPageLayoutDesc = 'Password OTP Form Page (Commerzbank Cobrands)';
...
</div>' where fk_id_layout = @idPage;

SET @pageLayoutIdHelp = (SELECT id FROM `CustomPageLayout` WHERE `DESCRIPTION` = 'Help Page (Commerzbank Cobrands)' );

delete from CustomPageLayout_ProfileSet
where customPageLayout_id = @pageLayoutIdHelp;

delete from CustomComponent where fk_id_layout = @pageLayoutIdHelp;

delete from CustomPageLayout where description = 'Help Page (Commerzbank Cobrands)';
...
set @pageLayoutIdSMS = (select id from `CustomPageLayout` where `pageType` = 'OTP_SMS_APP_VIEW' and DESCRIPTION = 'SMS_App_View (COB)');
...
</div>' where fk_id_layout = @passwordPageLayoutId;

delete from CustomItem where fk_id_customItemSet = @customItemSetSMS and pageTypes = 'HELP_PAGE' and ordinal in (1,2,3,11);
