...
SET @profileAccept = (SELECT id FROM Profile WHERE name = CONCAT(@BankUB, '_ACCEPT_01'));
SET @profileRefusalDefault = (SELECT id FROM Profile WHERE name = CONCAT(@BankUB, '_REFUSAL_01'));
SET @subIssuerID = (SELECT id FROM `SubIssuer` WHERE `code` = '16901' AND `name` = 'BNP Paribas Wealth Management');
SET @fraudRuleConditionId = (SELECT id FROM RuleCondition WHERE `name`=CONCAT('C1_P_', @BankUB, '_05_FRAUD'));
...
SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` = 'PS_BNP_WM_01');
SET @passwordCustomItemSet = (SELECT id FROM CustomItemSet WHERE name = CONCAT('customitemset_',@BankUB,'_MOBILE_APP_EXT'));
...
START TRANSACTION;
DELETE FROM CustomItem WHERE fk_id_customItemSet = @passwordCustomItemSet AND ordinal = 10 AND value = @textValue;
...
DELETE FROM TransactionValue WHERE fk_id_condition = @photoTanRuleConditionId;
DELETE FROM Condition_TransactionStatuses WHERE id_condition = @fraudRuleConditionId;
...
DELETE FROM Rule WHERE fk_id_profile = @profileUNDEFINED AND name = 'UNDEFINED';
DELETE FROM Profile WHERE fk_id_subIssuer = @subIssuerID AND name = CONCAT(@BankUB,'_UNDEFINED_01');
COMMIT;



UPDATE Rule SET orderRule = 8 WHERE name = 'ACCEPT MAINT' AND fk_id_profile = @profileAccept;
UPDATE Rule SET orderRule = 9 WHERE name = 'REFUSAL (DEFAULT)' AND fk_id_profile = @profileRefusalDefault;
