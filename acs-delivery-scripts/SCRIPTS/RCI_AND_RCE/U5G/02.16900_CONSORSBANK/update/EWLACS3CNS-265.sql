...
('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@authentMean,'_',@polingPageType,'_10'), @updateState,
 'de', 10, @polingPageType,	 @textValue, @MaestroVID, NULL, @passwordCustomItemSet);
...
UPDATE Rule SET orderRule = 7 WHERE name = 'ACCEPT MAINT' AND fk_id_profile = @profileAccept;
...
SET @ruleConditionPhotoTan = (SELECT `id` FROM `RuleCondition` WHERE `fk_id_rule` = @rulePhotoTan AND name = 'C1_P_BNP_WM_02_PHOTO_TAN');
