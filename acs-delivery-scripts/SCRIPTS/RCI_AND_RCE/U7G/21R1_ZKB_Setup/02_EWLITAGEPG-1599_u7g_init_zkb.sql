...
SET @availableAuthMeans = 'REFUSAL|OTP_SMS|MOBILE_APP_EXT|INFO|UNDEFINED';
...
(@createdBy, NOW(), 'OTP_SMS (FALLBACK)', NULL, NULL, 'OTP_SMS (FALLBACK)', @updateState, 8, @profileSMS),
(@createdBy, NOW(), 'OTP_SMS (BACKUP)', NULL, NULL, 'OTP_SMS (BACKUP)', @updateState, 9, @profileSMS),
...
SET @ruleSMSFallBack = (SELECT id FROM `Rule` WHERE `description` = 'OTP_SMS (FALLBACK)' AND `fk_id_profile` = @profileSMS);
SET @ruleSMSBackUP = (SELECT id FROM `Rule` WHERE `description` = 'OTP_SMS (BACKUP)' AND `fk_id_profile` = @profileSMS);
...
(@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_', @BankUB, '_01_OTP_SMS'), @updateState, @ruleSMSFallBack),
(@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_', @BankUB, '_02_OTP_SMS'), @updateState, @ruleSMSFallBack),
(@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_', @BankUB, '_01_OTP_SMS_BACKUP'), @updateState, @ruleSMSBackUP),
(@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_', @BankUB, '_02_OTP_SMS_BACKUP'), @updateState, @ruleSMSBackUP),
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
WHERE c.`name` = CONCAT('C1_P_', @BankUB, '_01_OTP_SMS')
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
WHERE c.`name` = CONCAT('C1_P_', @BankUB, '_02_OTP_SMS')
  AND mps.`fk_id_authentMean` = @authMeanOTPsms
  AND (mps.`meansProcessStatusType` = 'MEANS_AVAILABLE' AND mps.`reversed` = FALSE);
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
WHERE c.`name` = CONCAT('C1_P_', @BankUB, '_01_OTP_SMS_BACKUP')
  AND mps.`fk_id_authentMean` = @authMeanOTPsms
  AND (mps.`meansProcessStatusType` = 'MEANS_AVAILABLE' AND mps.`reversed` = FALSE);
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
WHERE c.`name` = CONCAT('C1_P_', @BankUB, '_02_OTP_SMS_BACKUP')
  AND mps.`fk_id_authentMean` = @authentMeansMobileAppExt
  AND (mps.`meansProcessStatusType` = 'MEANS_AVAILABLE' AND mps.`reversed` = FALSE);
