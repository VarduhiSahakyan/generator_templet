...
DELETE FROM `CustomItem` WHERE fk_id_customItemSet IN (@customItemSetRefusalFraud, @customItemSetBackupRefusal);
...
('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_220'), @updateState,
 'de', 220, @refusalPageType, 'Zahlung nicht ausgeführt', @MaestroVID, NULL, @customItemSetDefaultRefusal),

('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_230'), @updateState,
 'de', 230, @refusalPageType, 'Aus Sicherheitsgründen wurde Ihre Transaktion nicht ausgeführt. Ihre Karte wurde nicht belastet.', @MaestroVID, NULL, @customItemSetDefaultRefusal),

('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_220'), @updateState,
 'en', 220, @refusalPageType, 'Payment not completed', @MaestroVID, NULL, @customItemSetDefaultRefusal),

('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_230'), @updateState,
 'en', 230, @refusalPageType, 'For security reasons, your transaction was not completed and your card not debited.', @MaestroVID, NULL, @customItemSetDefaultRefusal),

('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_220'), @updateState,
 'fr', 220, @refusalPageType, 'Le paiement n''a pas été effectué', @MaestroVID, NULL, @customItemSetDefaultRefusal),

('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_230'), @updateState,
 'fr', 230, @refusalPageType, 'Pour des raisons de sécurité, votre transaction n''a pas été exécutée. Votre carte n''a pas été débitée.', @MaestroVID, NULL, @customItemSetDefaultRefusal),

('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_220'), @updateState,
 'it', 220, @refusalPageType, 'Pagamento non eseguito', @MaestroVID, NULL, @customItemSetDefaultRefusal),

('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@refusalAuthentMean,'_',@refusalPageType,'_230'), @updateState,
 'it', 230, @refusalPageType, 'Per ragioni di sicurezza, la tua transazione non è stata eseguita. La sua carta non è stata addebitata.', @MaestroVID, NULL, @customItemSetDefaultRefusal);



SET @authMeanREFUSAL = (SELECT id FROM `AuthentMeans` WHERE `name` = 'REFUSAL');
UPDATE `Profile` SET  `fk_id_AuthentMeans` = @authMeanREFUSAL, `fk_id_customItemSetCurrent` = NULL WHERE `id` = @profileBackupRefusal;

UPDATE `Profile` SET  `fk_id_customItemSetCurrent` = NULL WHERE `id` = @profileRefusalFraud;

DELETE FROM `CustomItemSet` WHERE `fk_id_subIssuer` = @subIssuerID AND `name` IN (CONCAT('customitemset_', @BankUB, '_REFUSAL_FRAUD'), CONCAT('customitemset_', @BankUB, '_BACKUP_REFUSAL'));
