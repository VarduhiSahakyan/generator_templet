...
SET @profileBackupRefusal = (SELECT `id` FROM `Profile` WHERE `name` = 'ZKB_BACKUP_REFUSAL');
...
SET @customItemSetRefusalFraud = (SELECT `id` FROM `CustomItemSet` WHERE `name` = 'customitemset_ZKB_REFUSAL_FRAUD');
