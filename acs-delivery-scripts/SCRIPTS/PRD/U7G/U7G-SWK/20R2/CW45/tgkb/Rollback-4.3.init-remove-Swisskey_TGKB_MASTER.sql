...
delete from CustomItem where FK_ID_CUSTOMITEMSET IS NULL OR id in (select id from (select ci.id from CustomItem ci,
                                                                                   ...
                                                                                   where ci.FK_ID_CUSTOMITEMSET = cis.ID
                                                                                     ...
                                                                                     and pr.ID_PROFILESET in
                                                                                         (@id_profile_set_tgkb)
                                                                                     and (ci.fk_id_network is NULL or ci.fk_id_network = @NetworkMID)
                                                                                  ) as temp);


delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET in (@id_profile_set_tgkb)
                                                                               and fk_id_network = @NetworkMID);

delete from BinRange where FK_ID_PROFILESET in (@id_profile_set_tgkb)
                       and fk_id_network = @NetworkMID;
-- delete subissuer and issuer

delete from Network_SubIssuer where ID_SUBISSUER in (@id_subissuer_tgkb)
                                and id_network = @NetworkMID;
delete from SubIssuerNetworkCrypto where FK_ID_SUBISSUER in (@id_subissuer_tgkb)
                                     and fk_id_network = @NetworkMID;
