...
SET @SubIssuerCodeRCH = '76700';

set @id_issuer = (SELECT ID from Issuer where CODE = @issuerCode);

set @id_subIssuer = (SELECT ID from SubIssuer where FK_ID_ISSUER = @id_issuer and code = @SubIssuerCodeRCH);
...
delete from CustomPageLayout_ProfileSet where PROFILESET_ID in (@id_profile_set);
...
set @id_customitemsets = (select group_concat(cis.id) from CustomItemSet cis,
                          ...
                          where	 p.FK_ID_CUSTOMITEMSETCURRENT = cis.id
                            ...
                            and pr.ID_RULE = r.ID
                            and pr.ID_PROFILESET in (@id_profile_set));
...
where pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set);

SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set);
...
delete from RuleCondition where ID in (SELECT id from (SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
                                                       where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set)) as temp);
...
delete from ProfileSet_Rule where ID_PROFILESET in (@id_profile_set);

delete from Rule where find_in_set(id, @id_rule);

delete from Profile where find_in_set(id, @id_profile);

delete from ProfileSet where id in (@id_profile_set);

delete from MerchantPivotList where FK_ID_ISSUER = @id_issuer;

delete from SubIssuer where ID in (@id_subIssuer);

delete from Issuer where ID = @id_issuer;

delete from Image where name in (@BankUB);

SET @cryptoConfigID = (SELECT id FROM CryptoConfig where description = 'CryptoConfig for BCV');
