...
SET @authentMeanRefusal = (SELECT `id` FROM `AuthentMeans` WHERE `name` = 'INFO');
SET @profileBackupRefusal = (SELECT `id` FROM `Profile` WHERE `name` = 'RCH_BACKUP_REFUSAL');
SET @description = 'INFO';

UPDATE `Profile` SET `fk_id_AuthentMeans` = @authentMeanRefusal,
                     `description` = @description WHERE `id` = @profileBackupRefusal;

SET @customItemSetRefusalFraud = (SELECT `id` FROM `CustomItemSet` WHERE `name` = 'customitemset_RCH_REFUSAL_FRAUD');

UPDATE `CustomItem` SET `ordinal` = 22 WHERE `fk_id_customItemSet` = @customItemSetRefusalFraud AND `ordinal` = 220;
UPDATE `CustomItem` SET `ordinal` = 23 WHERE `fk_id_customItemSet` = @customItemSetRefusalFraud AND `ordinal` = 230;

set @customPageLayoutDesc = 'Refusal Page (Raiffeisen)';
