...
SELECT 'T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(n.code,@authentMean,@pageTypes,'_',@ordinalMessage), @updateState, @locale, @ordinalMessage,  @pageTypes, 'Bitte aktivieren Sie Ihre Kreditkarte im Online Banking oder wenden Sie sich an den Ansprechpartner in Ihrer Filiale.', NULL, NULL, @customItemSetRefusal FROM `Network` n WHERE  n.id in (@networkVISA);
