...
SET @IssuerB = 'Commerzbank Cobrands';
...
/*sssss*/
INSERT INTO `Customer` (`code`, `customerType`, `name`, `parent_id`)
  SELECT  '19450', 'ISSUER', @IssuerB,   c.id FROM `Customer` c WHERE c.name=@BankB AND c.customerType='ENTITY';
/* This next command should be executed only if there is no migration process (ACS1 > ACS3) */
INSERT INTO `Customer` (`code`, `customerType`, `name`, `parent_id`)
  SELECT '19450', 'SUB_ISSUER', @IssuerB,  c.id FROM `Customer` c WHERE c.name=@IssuerB AND c.customerType='ISSUER';
