...
SET @availableAuthMeans = 'OTP_SMS|REFUSAL|PWD_OTP|PASSWORD';
SET @issuerNameAndLabel = 'Commerzbank Cobrands';
...
SET @subIssuerNameAndLabel = 'Commerzbank Cobrands';
...
INSERT INTO `SubIssuer` (`acsId`, `authenticationTimeOut`, `backupLanguages`, `code`, `codeSvi`, `currencyCode`,
                         ...
                         `issuerCountry`, `hubCallMode`, `fk_id_issuer`, `maskParams`, `dateFormat`, `resendSameOTP`) VALUES
  ('ACS_U5G', 120, @backUpLanguages, @subIssuerCode, @subIssuerCode, '978', @createdBy, NOW(), NULL, NULL, NULL, @subIssuerNameAndLabel,
   ...
   @issuerCountryCode, @HUBcallMode, @issuerId, @maskParam, @dateFormat, FALSE);
...
INSERT INTO AuthentMeans (createdBy, creationDate, description, lastUpdateBy, lastUpdateDate, name, updateState)
  SELECT 'InitPhase', sysdate(), 'ACCEPT', NULL, NULL, 'ACCEPT', 'PUSHED_TO_CONFIG' FROM dual
  WHERE NOT EXISTS (SELECT id FROM AuthentMeans where name = 'ACCEPT');
...
SET @authMeanPasswordOTP = (SELECT id FROM `AuthentMeans` WHERE `name` = 'PWD_OTP');
...
INSERT INTO `Profile` (`createdBy`, `creationDate`, `description`, `lastUpdateBy`, `lastUpdateDate`, `name`,
  ...
  (@createdBy, NOW(), 'RBA_ACCEPT', NULL, NULL, CONCAT(@BankUB,'_ACCEPT'), 'PUSHED_TO_CONFIG', 3, @authMeanINFO, @customItemSetRBAACCEPT, NULL, NULL, @subIssuerID),
  (@createdBy, NOW(), 'RBA_DECLINE', NULL, NULL, CONCAT(@BankUB,'_DECLINE'), 'PUSHED_TO_CONFIG', 3, @authMeanINFO, @customItemSetRBADECLINE, NULL, NULL, @subIssuerID),
  ...
  (@createdBy, NOW(), CONCAT(@Bank_B,'PASSWORD (NORMAL)'), NULL, NULL, CONCAT(@BankUB,'_PASSWORD_01'), 'PUSHED_TO_CONFIG', 2, @authMeanPasswordOTP, NULL, NULL, NULL, @subIssuerID),
  (@createdBy, NOW(), CONCAT(@Bank_B,'PASSWORD (PWD+OTP OVERRIDE)'), NULL, NULL, CONCAT(@BankUB,'_PASSWORD_02'), 'PUSHED_TO_CONFIG', 2, @authMeanPassword, @customItemSetPassword, NULL, NULL, @subIssuerID);

/* Rule */
/*!40000 ALTER TABLE `Rule` DISABLE KEYS */;
SET @profileRefusal = (SELECT id FROM `Profile` WHERE `name` = CONCAT(@BankUB,'_DEFAULT_REFUSAL'));
SET @profilePasswordOTP = (SELECT id FROM `Profile` WHERE `name` = CONCAT(@BankUB,'_PASSWORD_01'));
...
INSERT INTO `Rule` (`createdBy`, `creationDate`, `description`, `lastUpdateBy`, `lastUpdateDate`, `name`,
  ...
  (@createdBy, NOW(), 'PASSWORD_AVAILABLE_NORMAL', NULL, NULL, 'PWD_OTP (NORMAL)', 'PUSHED_TO_CONFIG', 4, @profilePasswordOTP),
  (@createdBy, NOW(), 'PASSWORD_AVAILABLE_OVERRIDE', NULL, NULL, 'PASSWORD (PWD+OTP OVERRIDE)', 'PUSHED_TO_CONFIG', 5, @profilePassword),
  (@createdBy, NOW(), 'SMS_AVAILABLE_OVERRIDE', NULL, NULL, 'OTP_SMS (PWD+OTP OVERRIDE)', 'PUSHED_TO_CONFIG', 6, @profileSMS),
  (@createdBy, NOW(), 'REFUSAL_DEFAULT', NULL, NULL, 'REFUSAL (DEFAULT)', 'PUSHED_TO_CONFIG', 7, @profileRefusal);
...
SET @rulePasswordnormal = (SELECT id FROM `Rule` WHERE `description`='PASSWORD_AVAILABLE_NORMAL' AND `fk_id_profile`=@profilePasswordOTP);
SET @rulePasswordOverride = (SELECT id FROM `Rule` WHERE `description`='PASSWORD_AVAILABLE_OVERRIDE' AND `fk_id_profile`=@profilePassword);
SET @ruleSMSnormal = (SELECT id FROM `Rule` WHERE `description`='SMS_AVAILABLE_OVERRIDE' AND `fk_id_profile`=@profileSMS);
...
INSERT INTO `RuleCondition` (`createdBy`, `creationDate`, `description`, `lastUpdateBy`, `lastUpdateDate`, `name`,
  ...
  (@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE'), 'PUSHED_TO_CONFIG', @ruleSMSnormal),
  (@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_',@BankUB,'_01_PASSWORD_NORMAL'), 'PUSHED_TO_CONFIG', @rulePasswordnormal),
  (@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_',@BankUB,'_01_PASSWORD_OVERRIDE'), 'PUSHED_TO_CONFIG', @rulePasswordOverride),
...
INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE') AND (ts.`transactionStatusType`='KNOWN_PHONE_NUMBER' AND ts.`reversed`=FALSE);

INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE') AND (ts.`transactionStatusType`='ERROR_DURING_OTP_SMS_SENDING' AND ts.`reversed`=TRUE);

INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE') AND (ts.`transactionStatusType`='ALWAYS_ACCEPT' AND ts.`reversed`=TRUE);

INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE') AND (ts.`transactionStatusType`='ALWAYS_DECLINE' AND ts.`reversed`=TRUE);
...
INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_PASSWORD_NORMAL') AND (ts.`transactionStatusType`='PHONE_NUMBER_IN_NEGATIVE_LIST' AND ts.`reversed`=TRUE);

INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_PASSWORD_OVERRIDE') AND (ts.`transactionStatusType`='ALWAYS_ACCEPT' AND ts.`reversed`=TRUE);

INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_PASSWORD_OVERRIDE') AND (ts.`transactionStatusType`='ALWAYS_DECLINE' AND ts.`reversed`=TRUE);

INSERT INTO `Condition_TransactionStatuses` (`id_condition`, `id_transactionStatuses`)
  SELECT c.id, ts.id FROM `RuleCondition` c, `TransactionStatuses` ts
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_PASSWORD_OVERRIDE') AND (ts.`transactionStatusType`='PHONE_NUMBER_IN_NEGATIVE_LIST' AND ts.`reversed`=TRUE);
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  ...
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE')
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE')
    AND mps.`fk_id_authentMean`=@authMeanPasswordOTP AND (mps.`meansProcessStatusType`='MEANS_AVAILABLE' AND mps.`reversed`=FALSE);

INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE')
    AND mps.`fk_id_authentMean`=@authMeanPasswordOTP
    AND (mps.`meansProcessStatusType` IN ('MEANS_DISABLED') AND mps.`reversed`=TRUE);
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE')
    AND mps.`fk_id_authentMean`=@authMeanPasswordOTP AND (mps.`meansProcessStatusType`='HUB_AUTHENTICATION_MEAN_AVAILABLE' AND mps.`reversed`=FALSE);

INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE')
    AND mps.`fk_id_authentMean`=@authMeanPasswordOTP AND (mps.`meansProcessStatusType`='PARENT_AUTHENTICATION_MEAN_AVAILABLE' AND mps.`reversed`=FALSE);

INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE')
    AND mps.`fk_id_authentMean`=@authMeanPasswordOTP AND (mps.`meansProcessStatusType`='USER_CHOICE_DEMANDED' AND mps.`reversed`=TRUE);
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_OTP_SMS_OVERRIDE')
    AND mps.`fk_id_authentMean`=@authMeanPassword AND (mps.`meansProcessStatusType`='USER_CHOICE_DEMANDED' AND mps.`reversed`=TRUE);
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_PASSWORD_NORMAL')
    AND mps.`fk_id_authentMean`=@authMeanPasswordOTP AND (mps.`meansProcessStatusType`='PARENT_AUTHENTICATION_MEAN_AVAILABLE' AND mps.`reversed`=TRUE);
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name`= CONCAT('C1_P_',@BankUB,'_01_PASSWORD_OVERRIDE')
    AND mps.`fk_id_authentMean`=@authMeanPasswordOTP AND (mps.`meansProcessStatusType`='MEANS_AVAILABLE' AND mps.`reversed`=FALSE);
...
INSERT INTO `ProfileSet_Rule` (`id_profileSet`, `id_rule`)
    ...
    WHERE ps.`name` = CONCAT('PS_',@BankUB,'_01') AND r.`id` IN (@ruleRefusalFraud, @ruleRBAAccept, @ruleRBADecline, @ruleSMSnormal, @rulePasswordnormal, @rulePasswordOverride, @ruleRefusalDefault);
