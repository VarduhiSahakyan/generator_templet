...
SET @pageLayoutIdSMS = (SELECT id FROM `CustomPageLayout` WHERE `pageType` = 'OTP_FORM_PAGE' AND DESCRIPTION = 'OTP Form Page (Commerzbank Cobrands)');
...
SET @pageLayoutInfoRefusal = (SELECT id FROM `CustomPageLayout` WHERE `pageType` = 'INFO_REFUSAL_PAGE' AND DESCRIPTION = 'INFO Refusal Page (Commerzbank Cobrands)');
...
SET @pageLayoutRefusal = (SELECT id FROM `CustomPageLayout` WHERE `pageType` = 'REFUSAL_PAGE' AND DESCRIPTION = 'Refusal Page (Commerzbank Cobrands)');
...
<div id="optGblPage">
	...
	<div id="footer"></div>
</div>' where fk_id_layout = @pageLayoutRefusal;


SET @pageLayoutFailure = (SELECT id FROM `CustomPageLayout` WHERE `pageType` = 'FAILURE_PAGE' AND DESCRIPTION = 'Failure Page (Commerzbank Cobrands)');
