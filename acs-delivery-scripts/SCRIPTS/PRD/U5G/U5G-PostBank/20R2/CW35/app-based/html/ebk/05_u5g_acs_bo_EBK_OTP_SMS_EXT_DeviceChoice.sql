...
( NULL, 'OTP_SMS_EXT_MESSAGE_APP_VIEW_DEVICE_SELECT', 'SMS_DEVICE_Choice_App_View (EBK)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_18501_PB_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'OTP_SMS_EXT_MESSAGE_APP_VIEW_DEVICE_SELECT' and DESCRIPTION = 'SMS_DEVICE_Choice_App_View (EBK)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'SMS_DEVICE_Choice_App_View (EBK)' and pageType = 'OTP_SMS_EXT_MESSAGE_APP_VIEW_DEVICE_SELECT' and ps.name = 'PS_18501_PB_01';
