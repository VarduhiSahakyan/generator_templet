...
( NULL, 'OTP_SMS_EXT_MESSAGE_APP_VIEW', 'SMS_App_View (EBK)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_18501_PB_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'OTP_SMS_EXT_MESSAGE_APP_VIEW' and DESCRIPTION = 'SMS_App_View (EBK)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'SMS_App_View (EBK)' and pageType = 'OTP_SMS_EXT_MESSAGE_APP_VIEW' and ps.name = 'PS_18501_PB_01';
