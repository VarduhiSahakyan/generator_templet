...
set @Sharedid_profile_set = (SELECT ID from ProfileSet where FK_ID_SUBISSUER = @sharedSubIssuerID);
...
delete from CustomPageLayout_ProfileSet where PROFILESET_ID in (@id_profile_set,@Sharedid_profile_set);
...
set @id_customitemsets = (select group_concat(cis.id) from CustomItemSet cis,
                          ...
                          where  p.FK_ID_CUSTOMITEMSETCURRENT = cis.id
                                 ...
                                 and pr.ID_RULE = r.ID
                                 and pr.ID_PROFILESET in (@id_profile_set,@Sharedid_profile_set));
...
SELECT r.id from Rule r, ProfileSet_Rule pr
                       where pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set,@Sharedid_profile_set);


SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
                       where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set,@Sharedid_profile_set);
...
delete from RuleCondition where ID in (SELECT id from (SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
                       where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set,@Sharedid_profile_set)) as temp);
...
delete from ProfileSet_Rule where ID_PROFILESET in (@id_profile_set,@Sharedid_profile_set);
delete from Rule where find_in_set(id, @id_rule);

delete from ProfileSet where id in (@id_profile_set,@Sharedid_profile_set);
delete from MerchantPivotList where FK_ID_SUBISSUER in ( @id_subissuer, @sharedSubIssuerID);
-- delete subissuer and issuer

delete from SubIssuerCrypto where FK_ID_SUBISSUER in (@id_subissuer,@sharedSubIssuerID);
delete from SubIssuer where ID in (@id_subissuer,@sharedSubIssuerID);
