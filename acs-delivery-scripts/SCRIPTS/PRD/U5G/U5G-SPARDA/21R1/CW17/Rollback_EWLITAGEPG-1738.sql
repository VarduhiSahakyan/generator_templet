...
SET @cisSMSAPP = (SELECT `id` FROM `CustomItemSet` where name = 'customitemset_SPB_sharedBIN_SMS_APP');
SET @pageType = 'APP_VIEW_MEAN_SELECT';
SET @ordinal = 153;
SET @text = '';
UPDATE `CustomItem` SET `value` = @text WHERE `fk_id_customItemSet` = @cisSMSAPP AND
											  `ordinal` = @ordinal AND
											  `pageTypes` = @pageType;
