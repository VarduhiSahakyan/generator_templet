...
( NULL, 'MOBILE_APP_EXT_APP_VIEW_DEVICE_SELECT', 'MOBILE_APP_EXT_DEVICE_Choice_App_View (SPARDA)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_SPB_sharedBIN_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'MOBILE_APP_EXT_APP_VIEW_DEVICE_SELECT' and DESCRIPTION = 'MOBILE_APP_EXT_DEVICE_Choice_App_View (SPARDA)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'MOBILE_APP_EXT_DEVICE_Choice_App_View (SPARDA)' and pageType = 'MOBILE_APP_EXT_APP_VIEW_DEVICE_SELECT' and ps.name = 'PS_SPB_sharedBIN_01';
