...
UPDATE Issuer
SET availaibleAuthentMeans = 'OTP_SMS|REFUSAL|INFO|MOBILE_APP'
...
SET @undefinedRuleConditionName = (SELECT id FROM `RuleCondition`  WHERE `name`= CONCAT('C1_P_',@BankUB,'_01_UNDEFINED'));
SET @smsRuleConditionName = (SELECT id FROM `RuleCondition`	 WHERE `name`= CONCAT('C1_P_',@BankUB,'_REISEBANK_SMS_02'));
SET @subIssuerID = (SELECT id FROM `SubIssuer` WHERE `code` = @subIssuerCode AND `name` = @subIssuerName);
SET @smsRuleId = (SELECT id FROM `Rule` WHERE  `description`=CONCAT(@BankUB,'_OTP_SMS_CHOICE'));

DELETE FROM ProfileSet_Rule WHERE id_profileSet = @ProfileSet and id_rule = @RuleId;
DELETE FROM Condition_MeansProcessStatuses WHERE id_condition = @undefinedRuleConditionName;
DELETE FROM Condition_MeansProcessStatuses WHERE id_condition = @smsRuleConditionName;
DELETE FROM Condition_TransactionStatuses WHERE id_condition = @smsRuleConditionName;
DELETE FROM RuleCondition WHERE fk_id_rule = @RuleId;
DELETE FROM Rule WHERE fk_id_profile = @profileUndefined;
DELETE FROM RuleCondition WHERE id = @smsRuleConditionName;
DELETE FROM Profile WHERE name = CONCAT(@BankUB,'_UNDEFINED_01') AND fk_id_subIssuer = @subIssuerID;

COMMIT ;



DELETE FROM CustomItem WHERE fk_id_customItemSet = @customItemSetMobileApp AND name = CONCAT(@MaestroVName,'_',@currentAuthentMean,'_',@currentPageType,'_10');
