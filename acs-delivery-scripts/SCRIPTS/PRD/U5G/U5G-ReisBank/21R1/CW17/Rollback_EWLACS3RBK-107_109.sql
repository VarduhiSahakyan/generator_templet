...
SET @MasterCardId = (SELECT `id` FROM `Image` WHERE `name` = 'MC_LOGO');
SET @locale = 'de';
SET @ordinal = 20;
...
SET @name = CONCAT(@MaestroMName,'_',@currentAuthentMean,'_',@currentPageType,'_174');

DELETE FROM CustomItem WHERE fk_id_customItemSet = @customItemSetSMS AND name = @name
