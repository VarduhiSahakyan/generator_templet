...
WHERE c.`name`=CONCAT('C1_P_', @subIssuerCode, '_01_MISSING_AUTHENTICATION_REFUSAL_ReiseBank')
  AND mps.`fk_id_authentMean`=@authMeanMobileApp
  AND (mps.`meansProcessStatusType` IN ('HUB_AUTHENTICATION_MEAN_AVAILABLE') AND mps.`reversed`=TRUE);
...
('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@currentAuthentMean,'_',@currentPageType,'_15'), @updateState,
	'de', 15, @currentPageType, 'Der Bezahlvorgang wurde aus Sicherheitsgründen abgebrochen. Ihre Karte wird nicht belastet. Sie werden nun automatisch zur Händler-Webseite weitergeleitet.', @MaestroVID, NULL, @customItemSetMobileApp),
...
('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@currentAuthentMean,'_',@currentPageType,'_15'), @updateState,
	'en', 15, @currentPageType, 'The payment process was cancelled for security reasons. Your card will not be charged. You will be automatically redirected to the merchant website.', @MaestroVID, NULL, @customItemSetMobileApp),
('T', @createdBy, NOW(), NULL, NULL, NULL, CONCAT(@MaestroVName,'_',@currentAuthentMean,'_',@currentPageType,'_26'), @updateState,
...
INSERT INTO `CustomItem` (`DTYPE`, `createdBy`, `creationDate`, `description`, `lastUpdateBy`, `lastUpdateDate`, `name`, `updateState`,
 `locale`, `ordinal`, `pageTypes`, `value`, `fk_id_network`, `fk_id_image`, `fk_id_customItemSet`)
	SELECT 'I', @createdBy, NOW(), NULL, NULL, NULL, @bankLogo , @updateState, @locale, 251, @pageType, @smallLogo, null, im.id, @customItemSetId
	FROM `Image` im, `Network` n WHERE im.name LIKE CONCAT('%',@smallLogoName,'%') AND n.id = @networkMC;

INSERT INTO `CustomItem` (`DTYPE`, `createdBy`, `creationDate`, `description`, `lastUpdateBy`, `lastUpdateDate`, `name`, `updateState`,
 `locale`, `ordinal`, `pageTypes`, `value`, `fk_id_network`, `fk_id_image`, `fk_id_customItemSet`)
	SELECT 'I', @createdBy, NOW(), NULL, NULL, NULL, @bankLogo, @updateState, @locale, 252, @pageType, @mediumLogo, null, im.id, @customItemSetId
	FROM `Image` im, `Network` n WHERE im.name LIKE CONCAT('%',@mediumLogoName,'%') AND n.id = @networkMC;

INSERT INTO `CustomItem` (`DTYPE`, `createdBy`, `creationDate`, `description`, `lastUpdateBy`, `lastUpdateDate`, `name`, `updateState`,
 `locale`, `ordinal`, `pageTypes`, `value`, `fk_id_network`, `fk_id_image`, `fk_id_customItemSet`)
	SELECT 'I', @createdBy, NOW(), NULL, NULL, NULL, @bankLogo, @updateState, @locale, 253, @pageType, @largeLogo, null, im.id, @customItemSetId
