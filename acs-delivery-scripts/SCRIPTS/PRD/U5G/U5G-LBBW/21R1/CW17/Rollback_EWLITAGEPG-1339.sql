...
-- UPDATE `Profile` SET `fk_id_customItemSetCurrent` = @customItemSetSMS WHERE `name` = 'LBBW_SMS_01' AND `description` = 'SMS (OTP sent by SMS) OTP_SMS';
...
delete from RuleCondition where fk_id_rule in (@rulePasswordOTP, @rulePassword, @ruleSMS);
