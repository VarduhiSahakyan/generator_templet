...
SET @BankB = 'Paybox';
SET @IssuerCode = '18951';
SET @SubIssuerCode = '18951';


set @id_issuer = (SELECT ID from Issuer where CODE = @issuerCode);
set @id_subissuer_1 = (SELECT ID from SubIssuer where FK_ID_ISSUER = @id_issuer and code = @SubIssuerCode);
...
delete from CustomItem where FK_ID_CUSTOMITEMSET IS NULL OR id in (select id from (select ci.id from CustomItem ci,
                                                                                   ...
                                                                                   where ci.FK_ID_CUSTOMITEMSET = cis.ID
                                                                                     ...
                                                                                     and (ci.fk_id_network is NULL or ci.fk_id_network = @NetworkVID)) as temp);

delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET in (@id_profile_set_1) and fk_id_network = @NetworkVID);

delete from BinRange where FK_ID_PROFILESET in (@id_profile_set_1) and fk_id_network = @NetworkVID;

delete from Network_SubIssuer where ID_SUBISSUER in (@id_subissuer_1) and id_network = @NetworkVID;
...
delete from CustomItemSet where name like CONCAT('customitemset_', @BankB, '%');
...
delete from Image where name like  '%Paybox%';
