USE `U5G_ACS_BO`;

SET @createdBy = 'A758582';

SET @BankB_1 = 'Volkswagen';
SET @BankB_2 = 'Audi';
SET @IssuerCode = '19150';
SET @SubIssuerCode_1 = '19151';
SET @SubIssuerCode_2 = '19152';


set @id_issuer = (SELECT ID from Issuer where CODE = @issuerCode);
set @id_subissuer_1 = (SELECT ID from SubIssuer where FK_ID_ISSUER = @id_issuer and code = @SubIssuerCode_1);
set @id_subissuer_2 = (SELECT ID from SubIssuer where FK_ID_ISSUER = @id_issuer and code = @SubIssuerCode_2);

set @id_profile_set_1 = (SELECT ID from ProfileSet where FK_ID_SUBISSUER = @id_subissuer_1);
set @id_profile_set_2 = (SELECT ID from ProfileSet where FK_ID_SUBISSUER = @id_subissuer_2);

SET @NetworkVID = (SELECT `id` FROM `Network` WHERE `code` = 'VISA');
SET @NetworkMID = (SELECT `id` FROM `Network` WHERE `code` = 'MASTERCARD');

SET FOREIGN_KEY_CHECKS = 0;

-- delete profiles and custom items for sub issuer
delete from CustomItem where FK_ID_CUSTOMITEMSET IS NULL OR id in (select id from (select ci.id from CustomItem ci,
                                                                                                     CustomItemSet cis,
                                                                                                     Profile p,
                                                                                                     Rule r,
                                                                                                     ProfileSet_Rule pr
                                                                                   where ci.FK_ID_CUSTOMITEMSET = cis.ID
                                                                                     and p.FK_ID_CUSTOMITEMSETCURRENT = cis.id
                                                                                     and r.FK_ID_PROFILE = p.ID
                                                                                     and pr.ID_RULE = r.ID
                                                                                     and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2)
                                                                                     and (ci.fk_id_network is NULL or ci.fk_id_network = @NetworkVID or ci.fk_id_network = @NetworkMID)) as temp);

delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2) and fk_id_network in (@NetworkVID, @NetworkMID));

delete from BinRange where FK_ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2) and fk_id_network in (@NetworkVID, @NetworkMID);

delete from Network_SubIssuer where ID_SUBISSUER in (@id_subissuer_1, @id_subissuer_2) and id_network in (@NetworkVID, @NetworkMID);



delete from CustomComponent
where ID in (select id
             from (select CC.ID
                   from CustomComponent CC,
                        CustomPageLayout CP
                   where CC.FK_ID_LAYOUT = CP.ID
                     and (CP.DESCRIPTION like CONCAT('%(', @BankB_1, ')%')
                       or CP.DESCRIPTION like CONCAT('%(', @BankB_2, ')%')
                       or CP.DESCRIPTION like '%VW%')) as temp);

-- delete the profile set / custompage layout mapping
delete from CustomPageLayout_ProfileSet where PROFILESET_ID in (@id_profile_set_1, @id_profile_set_2);

delete from CustomPageLayout where DESCRIPTION like CONCAT('%(', @BankB_1, ')%')
                                or DESCRIPTION like CONCAT('%(', @BankB_2, ')%')
                                or DESCRIPTION like '%VW%';

-- delete profiles for sub issuer
set @id_customitemsets = (select group_concat(cis.id) from CustomItemSet cis,
                                                           Profile p,
                                                           Rule r ,
                                                           ProfileSet_Rule pr
                          where  p.FK_ID_CUSTOMITEMSETCURRENT = cis.id
                            and r.FK_ID_PROFILE = p.ID
                            and pr.ID_RULE = r.ID
                            and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2));

update Profile p set p.FK_ID_CUSTOMITEMSETCURRENT = null where find_in_set(FK_ID_CUSTOMITEMSETCURRENT, @id_customitemsets);

delete from CustomItemSet where find_in_set(id, @id_customitemsets);
delete from CustomItemSet where fk_id_subIssuer in (@id_subissuer_1, @id_subissuer_2);

set @id_profile = (select group_concat(p.id) from Profile p,
                                                  Rule r ,
                                                  ProfileSet_Rule pr
                   where  r.FK_ID_PROFILE = p.ID
                     and pr.ID_RULE = r.ID
                     and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2));

set @id_rule = (select group_concat(r.id) from Rule r , ProfileSet_Rule pr
                where  pr.ID_RULE = r.ID
                  and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2));

-- delete conditions
SELECT r.id from Rule r, ProfileSet_Rule pr
where pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2);

SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2);

delete from Condition_TransactionStatuses
where ID_CONDITION in (SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
                       where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2));

delete from Condition_MeansProcessStatuses
where ID_CONDITION in (SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
                       where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2));

delete from RuleCondition where ID in (SELECT id from (SELECT rc.id from RuleCondition rc, Rule r, ProfileSet_Rule pr
                                                       where rc.FK_ID_RULE = r.id and pr.ID_RULE = r.id and pr.ID_PROFILESET in
                                                                                                            (@id_profile_set_1, @id_profile_set_2)) as temp);

delete from Profile where find_in_set(id, @id_profile);

set @id_rule = (select group_concat(r.id) from Rule r , ProfileSet_Rule pr
                where  pr.ID_RULE = r.ID and pr.ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2));

-- update Profileset_Rule set ID_RULE = null where find_in_set(ID_RULE, @id_rule);

delete from ProfileSet_Rule where ID_PROFILESET in (@id_profile_set_1, @id_profile_set_2);

delete from Rule where find_in_set(id, @id_rule);

delete from ProfileSet where id in (@id_profile_set_1, @id_profile_set_2);

delete from MerchantPivotList where FK_ID_ISSUER = @id_issuer;

-- delete subissuer and issuer
delete from SubIssuer where ID in (@id_subIssuer_1, @id_subIssuer_2);

delete from Issuer where ID = @id_issuer;

delete from Image where name like  '%Audi%'
                                or name like '%Volkswagen%'
                                or name like '%VW%';


SET FOREIGN_KEY_CHECKS = 1;
