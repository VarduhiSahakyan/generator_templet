...
UPDATE `CustomComponent` cc SET cc.value = replace(cc.value, 'text-transform: uppercase;','') WHERE find_in_set(fk_id_layout, @idAppViewPage);

UPDATE `CustomComponent` cc SET cc.value = replace(cc.value, '.acs-purchase-context button,',
    '
.acs-purchase-context button{
    width: 100%;
    margin-bottom: 0.5em;
...
') WHERE find_in_set(fk_id_layout, @idAppViewPage);
