...
( NULL, 'OTP_SMS_APP_VIEW', 'SMS_App_View (VW)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_VW_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'OTP_SMS_APP_VIEW' and DESCRIPTION = 'SMS_App_View (VW)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'SMS_App_View (VW)' and pageType = 'OTP_SMS_APP_VIEW' and ps.name = 'PS_VW_01';
