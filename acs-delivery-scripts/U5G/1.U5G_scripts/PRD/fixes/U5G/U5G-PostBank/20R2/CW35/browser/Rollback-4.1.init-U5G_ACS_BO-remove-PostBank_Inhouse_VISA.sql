...
delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET in (@id_profile_set,@Sharedid_profile_set) and fk_id_network = @MaestroVID);

delete from BinRange where FK_ID_PROFILESET in (@id_profile_set,@Sharedid_profile_set) and fk_id_network = @MaestroVID;
