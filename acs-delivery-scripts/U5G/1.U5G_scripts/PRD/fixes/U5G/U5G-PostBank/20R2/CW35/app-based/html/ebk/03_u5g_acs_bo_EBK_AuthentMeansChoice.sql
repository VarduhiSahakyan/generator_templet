...
( NULL, 'UNDEFINED_APP_VIEW_MEAN_SELECT', 'Choice_App_View (EBK)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_18501_PB_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'UNDEFINED_APP_VIEW_MEAN_SELECT' and DESCRIPTION = 'Choice_App_View (EBK)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'Choice_App_View (EBK)' and pageType = 'UNDEFINED_APP_VIEW_MEAN_SELECT' and ps.name = 'PS_18501_PB_01';
