...
SET @customItemSetId = (SELECT id FROM `CustomItemSet` WHERE `name` = CONCAT('customitemset_','SPB_sharedBIN','_PASSWORD'));

UPDATE CustomItem SET VALUE = '<b>Kundennummer :</b>' WHERE pageTypes = @currentPageType AND fk_id_customItemSet in (@customItemSetId) AND ordinal = 3 and DTYPE='T';
...
('T', 'InitPhase', NOW(), NULL, NULL, NULL, CONCAT(@networkVISA,'_',@currentAuthentMean,'_',@currentPageType,'_5'), 'PUSHED_TO_CONFIG',
 'de', 5, @currentPageType, '@challenge1', NULL, NULL, @customItemSetId);
