...
( NULL, 'UNDEFINED_APP_VIEW_MEAN_SELECT', 'Choice_App_View (SPARDA)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_SPB_sharedBIN_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'UNDEFINED_APP_VIEW_MEAN_SELECT' and DESCRIPTION = 'Choice_App_View (SPARDA)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'Choice_App_View (SPARDA)' and pageType = 'UNDEFINED_APP_VIEW_MEAN_SELECT' and ps.name = 'PS_SPB_sharedBIN_01';
