-- deletes the whole etup for Consorsbank (Issuer Code 16900)
...
-- 1. delete from
set @id_issuer = (SELECT ID from Issuer where CODE = '16900');
...
where ID
      in (select id from (select CC.ID
          ...
          where CC.FK_ID_LAYOUT = CP.ID and CP.DESCRIPTION in
                                            ('Message Banner (Consorsbank)',
                                             'Help Page (Consorsbank)',
                                             'Hamburger Menu (Consorsbank)',
                                             'OTP Form Page (Consorsbank)',
                                             'Refusal Page (Consorsbank)',
											 'Failure Page (Consorsbank)',
											 'Choice Page (Consorsbank)')) as temp);
...
delete from Image where name in ('Consorsbank');
