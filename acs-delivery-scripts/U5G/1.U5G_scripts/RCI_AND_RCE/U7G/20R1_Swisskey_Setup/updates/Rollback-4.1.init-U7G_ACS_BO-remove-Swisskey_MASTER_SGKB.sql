...
delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET in (@id_profile_set_3)
                                                                               and fk_id_network = @NetworkID);

delete from BinRange where FK_ID_PROFILESET in (@id_profile_set_3)
                       and fk_id_network = @NetworkID;
-- delete subissuer and issuer

delete from Network_SubIssuer where ID_SUBISSUER in (@id_subissuer_3)
                                and id_network = @NetworkID;
delete from SubIssuerNetworkCrypto where FK_ID_SUBISSUER in (@id_subissuer_3)
                                     and fk_id_network = @NetworkID;
