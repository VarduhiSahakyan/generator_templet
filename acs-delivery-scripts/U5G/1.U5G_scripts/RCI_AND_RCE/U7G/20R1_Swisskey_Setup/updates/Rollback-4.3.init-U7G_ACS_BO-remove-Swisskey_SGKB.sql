...
where ID in (select id
             from (select CC.ID
                   ...
                   where CC.FK_ID_LAYOUT = CP.ID
                     and (
                           CP.DESCRIPTION like CONCAT('%(', @BankB, ')%')
                       )
                  ) as temp);
...
delete from CustomItemSet where name like CONCAT('customitemset_', @BankUB_03, '%');
...
delete from ProfileSet where id in (@id_profile_set_3);
