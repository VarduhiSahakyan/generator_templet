...
SET @locale = 'de';
SET @text = 'Zahlung nicht ausgeführt - Karte ist nicht für 3-D Secure registriert';
...
SET @text = 'Die Zahlung konnte nicht ausgeführt werden, da Ihre Karte nicht für 3-D Secure Zahlungen registriert ist. Sollten Sie den Kauf fortsetzen wollen, bitten wir Sie Ihre Karte gemäss der Anleitung Ihrer Bank entsprechend zu registrieren.';
...
SET @locale = 'en';
SET @text = 'Payment not completed – card is not registered for 3-D Secure.';
UPDATE `CustomItem` SET value = @text WHERE `ordinal` = 22
										AND pageTypes = @currentPageType
										AND locale = @locale
										AND fk_id_customItemSet =  @customItemSetId;
...
SET @text = 'The payment could not be completed because your card is not registered for 3-D Secure payments. If you wish to continue with the purchase, please register your card according the instructions issued by your bank.';
UPDATE `CustomItem` SET value = @text WHERE `ordinal` = 23
										AND pageTypes = @currentPageType
										AND locale = @locale
										AND fk_id_customItemSet =  @customItemSetId;
...
SET @text = 'Back to the online shop';
UPDATE `CustomItem` SET value = @text WHERE `ordinal` = 175
										AND pageTypes = @currentPageType
										AND locale = @locale
										AND fk_id_customItemSet =  @customItemSetId;
...
SET @locale = 'fr';
SET @text = 'Le paiement n''a pas été effectué - La carte n''est pas enregistrée pour 3-D Secure';
UPDATE `CustomItem` SET value = @text WHERE `ordinal` = 22
										AND pageTypes = @currentPageType
										AND locale = @locale
										AND fk_id_customItemSet =  @customItemSetId;
...
SET @text = 'Le paiement n''as pas pu être effectué car votre carte n''est pas enregistrée pour les paiements 3-D Secure. Si vous souhaitez poursuivre l''achat, nous vous prions d''enregistrer votre carte conformément aux instructions de votre banque.';
UPDATE `CustomItem` SET value = @text WHERE `ordinal` = 23
										AND pageTypes = @currentPageType
										AND locale = @locale
										AND fk_id_customItemSet =  @customItemSetId;
...
SET @locale = 'it';
SET @text = 'Pagamento non eseguito - La carta non è registrata per 3-D Secure';
UPDATE `CustomItem` SET value = @text WHERE `ordinal` = 22
										AND pageTypes = @currentPageType
										AND locale = @locale
										AND fk_id_customItemSet =  @customItemSetId;
...
SET @text = 'Non è stato possibile effettuare il pagamento perché la sua carta non è registrata per i pagamenti 3-D Secure. Se desidera continuare l’acquisto, la preghiamo di registrare la sua carta secondo le istruzioni della sua banca.';
UPDATE `CustomItem` SET value = @text WHERE `ordinal` = 23
										AND pageTypes = @currentPageType
										AND locale = @locale
										AND fk_id_customItemSet =  @customItemSetId;
