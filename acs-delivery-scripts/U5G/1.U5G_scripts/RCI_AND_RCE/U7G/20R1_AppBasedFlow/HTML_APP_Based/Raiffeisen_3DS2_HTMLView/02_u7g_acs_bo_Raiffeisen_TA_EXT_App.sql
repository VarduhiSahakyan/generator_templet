...
set @customPageLayoutDesc_appView = 'App_View TA (RAIFFEISEN)';
...
set @pageTypeAppExt = 'MOBILE_APP_EXT_APP_VIEW';
set @customPageLayoutDesc_appExtView = 'App_View_EXT_App_View (RAIFFEISEN)';

insert into CustomPageLayout (controller, pageType, description)
    values (null, @pageTypeAppExt, @customPageLayoutDesc_appExtView);

set @idAppEXTViewPage = (select id
                      from `CustomPageLayout`
                      where `pageType` = @pageTypeAppExt
                        and DESCRIPTION = @customPageLayoutDesc_appExtView);
