...
INSERT INTO `SubIssuer` (`acsId`, `authenticationTimeOut`, `backupLanguages`, `code`, `codeSvi`, `currencyCode`,
  ...
  ('ACS_U7G', 120, @backUpLanguages, @subIssuerCode, @subIssuerCode, '978', @createdBy, NOW(), NULL, NULL, NULL, @subIssuerNameAndLabel,
   ...
   @issuerCountryCode, @HUBcallMode, @issuerId, @maskParam, @dateFormat, 'https://u7g-3ds.wlp-acs.com/', '1', @3DS2AdditionalInfo, '3', TRUE, FALSE, b'0', b'0', @activatedAuthMeans, @cryptoConfigIDNAB, @currencyFormat);
...
(@createdBy, NOW(), 'OTP_SMS (FALLBACK)', NULL, NULL, 'OTP_SMS (FALLBACK)', @updateState, 6, @profileSMS),
...
INSERT INTO `RuleCondition` (`createdBy`, `creationDate`, `description`, `lastUpdateBy`, `lastUpdateDate`, `name`,
  ...
  (@createdBy, NOW(), NULL, NULL, NULL, CONCAT('C1_P_', @BankUB, '_01_OTP_SMS_FALLBACK'), @updateState, @ruleSMSnormal),
...
INSERT INTO `Condition_MeansProcessStatuses` (`id_condition`, `id_meansProcessStatuses`)
  SELECT c.id, mps.id FROM `RuleCondition` c, `MeansProcessStatuses` mps
  WHERE c.`name` = CONCAT('C1_P_', @BankUB, '_01_OTP_SMS_FALLBACK')
	AND mps.`fk_id_authentMean` = @authMeanOTPsms
	AND (mps.`meansProcessStatusType` = 'MEANS_AVAILABLE' AND mps.`reversed` = FALSE);
