...
delete from CustomItem where FK_ID_CUSTOMITEMSET IS NULL OR id in (select id from (select ci.id from CustomItem ci,
                                                                                   ...
                                                                                   where ci.FK_ID_CUSTOMITEMSET = cis.ID
                                                                                     ...
                                                                                     and (ci.fk_id_network is NULL or ci.fk_id_network = @NetworkMID)) as temp);

delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET in (@id_profile_set_1) and fk_id_network = @NetworkMID);

delete from BinRange where FK_ID_PROFILESET in (@id_profile_set_1) and fk_id_network = @NetworkMID;

delete from Network_SubIssuer where ID_SUBISSUER in (@id_subissuer_1) and id_network = @NetworkMID;

delete from SubIssuerNetworkCrypto where FK_ID_SUBISSUER in (@id_subissuer_1) and fk_id_network = @NetworkMID;
