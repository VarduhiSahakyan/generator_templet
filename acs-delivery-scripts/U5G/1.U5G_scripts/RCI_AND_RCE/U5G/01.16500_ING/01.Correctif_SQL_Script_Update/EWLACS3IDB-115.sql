...
set @customITemSetPassword= (SELECT `id` from `CustomItemSet` where `name`= 'customitemset_16500_PASSWORD' );
...
UPDATE `CustomItem`
SET `value` = ''
WHERE `ordinal` = 1 and `pageTypes`= @currentPageType  and `locale` = @locale and `fk_id_customItemSet`= @customITemSetPassword;
...
WHERE `ordinal` = 2 and `pageTypes`= @currentPageType  and `locale` = @locale and `fk_id_customItemSet`= @customITemSetPassword;