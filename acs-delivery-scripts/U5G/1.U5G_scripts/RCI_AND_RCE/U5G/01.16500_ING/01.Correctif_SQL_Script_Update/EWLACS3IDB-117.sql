...
UPDATE `CustomItem`
SET `value` = ''
WHERE `ordinal` = 1 and `pageTypes`= @currentPageType  and `locale` = @locale and `fk_id_customItemSet`= @customITemSetDefaultRefusal;
...
WHERE `ordinal` = 2 and `pageTypes`= @currentPageType  and `locale` = @locale and `fk_id_customItemSet`= @customITemSetDefaultRefusal;