...
WHERE c.`name` = @ruleConditionAuthenticationName
	AND mps.`fk_id_authentMean` = @idAuthMean
...
SET @IDMeansProcessStatuse = (SELECT id
							...
							WHERE meansProcessStatusType = 'HUB_AUTHENTICATION_MEAN_AVAILABLE'
								AND reversed = TRUE
								AND fk_id_authentMean = @authMeanMobileApp);
DELETE
...
SET @IDKnownPhoneNumber = (SELECT id
							FROM TransactionStatuses
							WHERE transactionStatusType = 'KNOWN_PHONE_NUMBER'
								AND reversed = FALSE);
...
WHERE id_condition = @ruleConditionId
	AND id_transactionStatuses = @IDKnownPhoneNumber;
 
...
SET @IDKnownPhoneNumberNL = (SELECT id
							FROM TransactionStatuses
							WHERE transactionStatusType = 'PHONE_NUMBER_IN_NEGATIVE_LIST'
								AND reversed = FALSE);
SET @ruleConditionRefusalFraud = 'C1_P_12000_REISEBANK_05_REFUSAL_FRAUD';
SET @ruleConditionId = (SELECT id
						FROM RuleCondition
						WHERE name = @ruleConditionRefusalFraud AND fk_id_rule = @ruleFraud);


DELETE
FROM Condition_TransactionStatuses
WHERE id_condition = @ruleConditionId
	AND id_transactionStatuses = @IDKnownPhoneNumberNL;
 

DELETE FROM RuleCondition WHERE name = @fraudRuleConditionName AND fk_id_rule = @ruleFraud;
...
WHERE name = 'PS_12232_REISEBANK_01';