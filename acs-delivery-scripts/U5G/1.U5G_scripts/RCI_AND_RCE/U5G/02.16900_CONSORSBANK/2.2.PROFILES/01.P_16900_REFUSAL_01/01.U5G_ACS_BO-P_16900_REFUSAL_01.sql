...
SET @customItemSetDescription = 'customitemset_16900_1_REFUSAL_Current';
SET @customItemSetName = 'customitemset_16900_1_REFUSAL';
SET @codeSubIssuer = '16900';

SET @idSubIssuer = (SELECT id FROM `SubIssuer` WHERE `code` = @codeSubIssuer);
SET @idAuthMeans = (SELECT id FROM `AuthentMeans` WHERE `name` = 'REFUSAL');
...
SET @profileName = '16900_REFUSAL_01';
SET @profileDescription = 'Authentication refused';
...
INSERT INTO `Profile` (`createdBy`, `creationDate`, `description`, `name`,
                       ...
                       `fk_id_subIssuer`)
VALUES  ('A169318', NOW(), @profileDescription, @profileName, 'PUSHED_TO_CONFIG', -1, '^[^OIi]*$', '8:(:DIGIT:1)',@idAuthMeans, @idCustomItemSet, NULL, NULL, @idSubIssuer);
