...
SET @customItemSetDescription = 'customitemset_16900_SMS_Current';
SET @customItemSetName = 'customitemset_16900_SMS';
SET @codeSubIssuer = '16900';

SET @idSubIssuer = (SELECT id FROM `SubIssuer` WHERE `code` = @codeSubIssuer);
SET @idAuthMeans = (SELECT id FROM `AuthentMeans` WHERE `name` = 'OTP_SMS_EXT_MESSAGE');
...
SET @profileName = '16900_SMS_01';
SET @profileDescription = 'Authentication by OTP SMS';
