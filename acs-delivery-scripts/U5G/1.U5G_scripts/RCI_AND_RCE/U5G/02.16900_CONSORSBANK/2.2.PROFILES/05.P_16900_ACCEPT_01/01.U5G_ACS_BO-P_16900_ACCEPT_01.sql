...
SET @customItemSetDescription = 'customitemset_16900_ACCEPT_MAINT_Current';
SET @customItemSetName = 'customitemset_16900_ACCEPT_MAINT';
SET @codeSubIssuer = '16900';

SET @idSubIssuer = (SELECT id FROM `SubIssuer` WHERE `code` = @codeSubIssuer);
SET @idAuthMeans = (SELECT id FROM `AuthentMeans` WHERE `name` = 'ACCEPT');
...
SET @profileName = '16900_ACCEPT_01';
SET @profileDescription = 'Automatic Authentication';
...
INSERT INTO `Profile` (`createdBy`, `creationDate`, `description`, `name`,
                       ...
                       `fk_id_subIssuer`)
VALUES  ('A169318', NOW(), @profileDescription, @profileName, 'PUSHED_TO_CONFIG', -1, @idAuthMeans, @idCustomItemSet, NULL, NULL, @idSubIssuer);
