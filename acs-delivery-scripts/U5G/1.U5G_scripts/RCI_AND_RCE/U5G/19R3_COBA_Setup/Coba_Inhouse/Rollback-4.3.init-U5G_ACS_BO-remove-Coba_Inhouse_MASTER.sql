...
delete from CustomItem where FK_ID_CUSTOMITEMSET IS NULL OR id in (select id from (select ci.id from CustomItem ci,
                                                     ...
                                                     and ci.fk_id_network = @MaestroMID
                                                     ) as temp);


delete from BinRange_SubIssuer where ID_BINRANGE in (select id from BinRange where FK_ID_PROFILESET = @id_profile_set and fk_id_network = @MaestroMID);

delete from BinRange where FK_ID_PROFILESET = @id_profile_set and fk_id_network = @MaestroMID;
-- delete subissuer and issuer

delete from Network_SubIssuer where ID_SUBISSUER = @id_subissuer and id_network = @MaestroMID;
delete from SubIssuerNetworkCrypto where FK_ID_SUBISSUER = @id_subissuer and fk_id_network = @MaestroMID;
