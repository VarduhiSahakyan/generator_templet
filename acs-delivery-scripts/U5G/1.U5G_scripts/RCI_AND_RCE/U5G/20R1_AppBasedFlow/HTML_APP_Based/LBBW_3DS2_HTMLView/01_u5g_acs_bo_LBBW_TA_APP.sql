...
( NULL, 'MOBILE_APP_APP_VIEW', 'TA_App_View (LBBW)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_LBBW_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'MOBILE_APP_APP_VIEW' and DESCRIPTION = 'TA_App_View (LBBW)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'TA_App_View (LBBW)' and pageType ='MOBILE_APP_APP_VIEW'and ps.name = 'PS_LBBW_01';
