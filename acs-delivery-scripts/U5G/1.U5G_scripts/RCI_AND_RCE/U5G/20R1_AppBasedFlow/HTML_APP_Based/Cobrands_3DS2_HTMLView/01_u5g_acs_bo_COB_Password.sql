...
( NULL, 'PASSWORD_APP_VIEW', 'PASSWORD_App_View (COB)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_COB_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'PASSWORD_APP_VIEW' and DESCRIPTION = 'PASSWORD_APP_VIEW (COB)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'PASSWORD_APP_VIEW (COB)' and pageType = 'PASSWORD_APP_VIEW' and ps.name = 'PS_COB_01';
