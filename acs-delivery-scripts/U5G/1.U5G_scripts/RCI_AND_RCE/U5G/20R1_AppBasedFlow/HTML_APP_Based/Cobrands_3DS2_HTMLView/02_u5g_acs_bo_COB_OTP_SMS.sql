...
( NULL, 'OTP_SMS_APP_VIEW', 'SMS_App_View (COB)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_COB_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'OTP_SMS_APP_VIEW' and DESCRIPTION = 'SMS_App_View (COB)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'SMS_App_View (COB)' and pageType = 'OTP_SMS_APP_VIEW' and ps.name = 'PS_COB_01';
