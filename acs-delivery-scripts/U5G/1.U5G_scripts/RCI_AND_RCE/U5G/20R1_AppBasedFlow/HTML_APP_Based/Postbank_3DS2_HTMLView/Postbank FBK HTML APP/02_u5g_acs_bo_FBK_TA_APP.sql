...
( NULL, 'MOBILE_APP_APP_VIEW', 'MOBILE_APP_APP_VIEW (FBK)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_18502_PB_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'MOBILE_APP_APP_VIEW' and DESCRIPTION = 'MOBILE_APP_APP_VIEW (FBK)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'MOBILE_APP_APP_VIEW (FBK)' and pageType ='MOBILE_APP_APP_VIEW'and ps.name = 'PS_18502_PB_01';
