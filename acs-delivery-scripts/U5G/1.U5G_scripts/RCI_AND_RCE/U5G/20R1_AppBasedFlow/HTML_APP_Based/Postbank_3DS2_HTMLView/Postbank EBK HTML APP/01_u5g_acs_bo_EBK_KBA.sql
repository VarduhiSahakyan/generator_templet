...
( NULL, 'KBA_APP_VIEW', 'KBA_APP_VIEW (EBK)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_18501_PB_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'KBA_APP_VIEW' and DESCRIPTION = 'KBA_APP_VIEW (EBK)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'KBA_APP_VIEW (EBK)' and pageType = 'KBA_APP_VIEW' and ps.name = 'PS_18501_PB_01';
