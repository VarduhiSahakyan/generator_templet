...
SET @idAppViewPageCHOICE=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'UNDEFINED_APP_VIEW_MEAN_SELECT' and DESCRIPTION = 'Choice_App_View (FBK)') ;
...
delete from CustomPageLayout where pageType like '%UNDEFINED_APP_VIEW_MEAN_SELECT%' and description like '%Choice_App_View (FBK)%';
