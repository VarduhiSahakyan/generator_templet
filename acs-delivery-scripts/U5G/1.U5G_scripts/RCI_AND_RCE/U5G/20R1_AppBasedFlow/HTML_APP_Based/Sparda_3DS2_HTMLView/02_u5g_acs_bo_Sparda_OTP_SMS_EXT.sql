...
( NULL, 'OTP_SMS_EXT_MESSAGE_APP_VIEW', 'SMS_App_View (SPARDA)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_SPB_sharedBIN_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'OTP_SMS_EXT_MESSAGE_APP_VIEW' and DESCRIPTION = 'SMS_App_View (SPARDA)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'SMS_App_View (SPARDA)' and pageType = 'OTP_SMS_EXT_MESSAGE_APP_VIEW' and ps.name = 'PS_SPB_sharedBIN_01';
