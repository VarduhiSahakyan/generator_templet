...
( NULL, 'EXT_PASSWORD_APP_VIEW', 'EXT_PASSWORD_App_View (SPARDA)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_SPB_sharedBIN_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'EXT_PASSWORD_APP_VIEW' and DESCRIPTION = 'EXT_PASSWORD_APP_VIEW (SPARDA)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'EXT_PASSWORD_APP_VIEW (SPARDA)' and pageType = 'EXT_PASSWORD_APP_VIEW' and ps.name = 'PS_SPB_sharedBIN_01';
