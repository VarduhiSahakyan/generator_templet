...
( NULL, 'EXT_PASSWORD_APP_VIEW', 'EXT_PASSWORD_App_View (COZ)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_COZ_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'EXT_PASSWORD_APP_VIEW' and DESCRIPTION = 'EXT_PASSWORD_APP_VIEW (COZ)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'EXT_PASSWORD_APP_VIEW (COZ)' and pageType = 'EXT_PASSWORD_APP_VIEW' and ps.name = 'PS_COZ_01';
