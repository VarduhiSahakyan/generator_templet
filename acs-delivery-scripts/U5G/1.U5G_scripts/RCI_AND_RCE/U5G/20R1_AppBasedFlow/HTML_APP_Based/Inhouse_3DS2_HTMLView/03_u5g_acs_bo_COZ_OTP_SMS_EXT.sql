...
( NULL, 'OTP_SMS_EXT_MESSAGE_APP_VIEW', 'SMS_App_View (COZ)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_COZ_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'OTP_SMS_EXT_MESSAGE_APP_VIEW' and DESCRIPTION = 'SMS_App_View (COZ)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'SMS_App_View (COZ)' and pageType = 'OTP_SMS_EXT_MESSAGE_APP_VIEW' and ps.name = 'PS_COZ_01';
