...
( NULL, 'MOBILE_APP_EXT_APP_VIEW', 'MOBILE_APP_EXT_App_View (COZ)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_COZ_01');

SET @idAppViewPage=(SELECT id FROM `CustomPageLayout` WHERE `pageType`= 'MOBILE_APP_EXT_APP_VIEW' and DESCRIPTION = 'MOBILE_APP_EXT_App_View (COZ)') ;
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = 'MOBILE_APP_EXT_App_View (COZ)' and pageType ='MOBILE_APP_EXT_APP_VIEW'and ps.name = 'PS_COZ_01';
