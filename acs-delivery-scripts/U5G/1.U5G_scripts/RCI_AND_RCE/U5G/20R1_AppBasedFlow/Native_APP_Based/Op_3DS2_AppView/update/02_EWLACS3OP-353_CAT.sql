...
( NULL,'APP_VIEW_LANGUAGE_SELECT', 'LANGUAGE_Choice_App_View (OP)');

SET @ProfileSet = (SELECT id FROM `ProfileSet` WHERE `name` ='PS_20000_SMS_OP');
...
select cpl.id, ps.id
  from CustomPageLayout cpl, ProfileSet ps
    where cpl.description = @customPageLayoutDesc_appView and pageType = @pageType  and ps.name = 'PS_20000_SMS_OP';
...
UPDATE `SubIssuer` SET `displayLanguageSelectPage` = true WHERE code = '20000';